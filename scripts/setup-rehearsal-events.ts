...
const configSchema = z.object({
  ...
  tables: z.array(z.object({ tableCode: z.string(), capacity: z.number().int().positive(), displayOrder: z.number().int().positive(), seats: z.array(z.object({ seatCode: z.string() })).min(1) })).min(1),
  dream: z.object({
    aiEnabled: z.boolean(), aiTimeoutMs: z.number().int(), projectConsentVersion: z.string().nullable(),
    fallbackBridgeTemplate: z.string(), fallbackCandidates: z.array(z.string()).length(3),
    cardSet: z.object({ code: z.string(), name: z.string(), version: z.number().int().positive(), cards: z.array(z.object({ name: z.string(), imageKey: z.string().nullable(), description: z.string().nullable() })).min(1) }),
  }),
  questionnaire: z.object({
    name: z.string(), version: z.number().int().positive(),
    questions: z.array(z.object({ axis: z.enum(["values", "marriage_intent", "relationship_pace", "conversation_style", "topic_overlap"]), prompt: z.string(), kind: z.enum(["multi_select", "ordinal", "complement"]), maxSelections: z.number().int().positive(), weight: z.number().int().positive(), options: z.array(z.object({ code: z.string(), label: z.string(), scoreValue: z.number().int().nullable() })).min(2) })).length(5),
  }),
  legalDocuments: z.array(z.object({ documentType: z.enum(["event_terms", "privacy"]), version: z.string(), title: z.string(), body: z.string() })).length(2),
...
const records = parse(await readFile(resolve(config.csvPath), "utf8"), { columns: true, bom: true, skip_empty_lines: true }) as Record<string, string>[];
const headers = Object.keys(records[0] ?? {});
const csvValue = (value: string) => /[",\r\n]/.test(value) ? `"${value.replaceAll('"', '""')}"` : value;
const toCsv = (rows: Record<string, string>[]) => `${headers.join(",")}\n${rows.map((row) => headers.map((header) => csvValue(row[header] ?? "")).join(",")).join("\n")}\n`;

let cookie = "";
const api = async (path: string, init?: RequestInit) => {
  const response = await fetch(new URL(path, config.baseUrl), { ...init, headers: { ...(init?.headers ?? {}), ...(cookie ? { cookie } : {}) } });
  const setCookie = response.headers.get("set-cookie");
  if (setCookie) cookie = setCookie.split(";", 1)[0] ?? cookie;
  const body = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(`API ${path} failed with HTTP ${response.status}: ${String((body as { code?: string }).code ?? "UNKNOWN")}`);
...
try {
  ...
  const tenantRows = await sql<{ id: string }[]>`select id from tenants where code = ${tenantCode} and status = 'active' limit 1`;
  const tenantId = tenantRows[0]?.id;
  if (!tenantId) throw new Error("Active rehearsal tenant was not found");

  const list = await api("/api/admin/events");
  const existing = new Map(((list.data ?? []) as Array<{ id: string; code: string }>).map((event) => [event.code, event.id]));
  const results: Array<Record<string, string | number>> = [];
  const activeTemplates = await sql<{ template_key: string }[]>`select template_key from notification_templates where tenant_id = ${tenantId} and active = true`;
  let templatesCreated = 0;
  for (const template of config.notificationTemplates) if (!activeTemplates.some((row) => row.template_key === template.templateKey)) {
    await api("/api/admin/platform/templates", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(template) });
    templatesCreated++;
  }
  ...
  for (const event of config.events) {
    ...
    if (!eventId) {
      const created = await api("/api/admin/events", {
        ...
        body: JSON.stringify({
          ...
          participantCategories: [{ code: "group_a", label: "グループA" }, { code: "group_b", label: "グループB" }],
    ...
    if (countsBefore[0]?.applications !== 4 || countsBefore[0]?.participants !== 4) {
      ...
      if (validatedData.error !== 0 || validated !== 4) throw new Error(`${event.code} CSV validation did not produce four committable rows`);
      const commit = await api(`/api/admin/events/${eventId}/imports/${validatedData.importId}/commit`, { method: "POST" });
    ...
    const currentTables = (tableResponse.data ?? []) as Array<{ tableCode: string; capacity: number; displayOrder: number; seats: Array<{ seatCode: string }> }>;
    const normalizedTables = (tables: typeof currentTables) => tables.map((table) => ({ tableCode: table.tableCode, capacity: table.capacity, displayOrder: table.displayOrder, seats: table.seats.map((seat) => ({ seatCode: seat.seatCode })).sort((a, b) => a.seatCode.localeCompare(b.seatCode)) })).sort((a, b) => a.displayOrder - b.displayOrder);
    if (currentTables.length === 0) await api(`/api/admin/events/${eventId}/tables`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(config.tables) });
    else if (JSON.stringify(normalizedTables(currentTables)) !== JSON.stringify(normalizedTables(config.tables))) throw new Error(`${event.code} already has a different table configuration`);
    ...
    const linkedQuestionnaires = await sql<{ code: string; question_count: number }[]>`
      ...
      group by q.code
    `;
    if (linkedQuestionnaires.length === 0) await api(`/api/admin/events/${eventId}/questionnaire-settings`, {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ code: event.questionnaireCode, ...config.questionnaire }),
    });
    else if (linkedQuestionnaires[0]?.code !== event.questionnaireCode || linkedQuestionnaires[0]?.question_count !== 5) throw new Error(`${event.code} already has a different questionnaire configuration`);
    ...
    `;
    for (const document of config.legalDocuments) if (!publishedDocuments.some((row) => row.document_type === document.documentType && row.version === document.version)) {
      const draft = await api(`/api/admin/events/${eventId}/legal-documents`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(document) });
      const documentId = (draft.data as { id: string }).id;
      await api(`/api/admin/events/${eventId}/legal-documents/${documentId}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "publish" }) });
    }

    const resourceCounts = await sql<{ table_count: number; seat_count: number; card_count: number; question_count: number; legal_count: number }[]>`
      select
    ...
    if (!resources || resources.table_count !== 2 || resources.seat_count !== 4 || resources.card_count !== 8 || resources.question_count !== 5 || resources.legal_count !== 2) throw new Error(`${event.code} master resource verification failed`);
    ...
    `;
    if (countsAfter[0]?.applications !== 4 || countsAfter[0]?.participants !== 4 || countsAfter[0]?.cross_event_references !== 0) throw new Error(`${event.code} isolation verification failed`);
    const refreshed = await api("/api/admin/events");
    const configured = ((refreshed.data ?? []) as Array<{ id: string; configuration?: { complete?: boolean; issues?: unknown[] } }>).find((item) => item.id === eventId)?.configuration;
    if (!configured?.complete) throw new Error(`${event.code} remains incomplete with ${configured?.issues?.length ?? "unknown"} configuration issues`);
    results.push({ code: event.code, eventId, validated, committed, participantsCreated: createdParticipants, applications: 4, participants: 4, crossEventReferences: 0, tables: 2, seats: 4, cards: 8, questions: 5, legalDocuments: 2, configurationIssues: 0 });
  }

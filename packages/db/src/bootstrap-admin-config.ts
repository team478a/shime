...
const environmentSchema = z.object({
  ...
  BOOTSTRAP_TENANT_CODE: z.string().trim().min(2).max(80).regex(/^[a-z0-9][a-z0-9-]*$/).default("shime"),
  BOOTSTRAP_TENANT_NAME: z.string().trim().min(1).max(200).default("SHIME"),
  BOOTSTRAP_ADMIN_LOGIN_ID: z.string().trim().min(3).max(255).regex(/^[a-z0-9][a-z0-9._-]*$/).default("admin"),
  ...
  if (env.APP_ENV === "production" && env.BOOTSTRAP_CONFIRM_PRODUCTION !== env.BOOTSTRAP_TENANT_CODE) {
    context.addIssue({ code: "custom", message: "Production bootstrap requires BOOTSTRAP_CONFIRM_PRODUCTION to equal BOOTSTRAP_TENANT_CODE" });
  }
});

...
export class LineProviderError extends Error {
  constructor(public readonly code: "INVALID_TOKEN" | "TOKEN_EXPIRED" | "PROVIDER_UNAVAILABLE" | "SEND_FAILED") {
    super(code);
  }
}
...
export class HttpLineProvider implements LineProvider {
  ...
  async verifyIdToken(idToken: string): Promise<VerifiedLineIdentity> {
    ...
    if (!body.sub) throw new LineProviderError("INVALID_TOKEN");
    if (body.exp && body.exp * 1000 <= Date.now()) throw new LineProviderError("TOKEN_EXPIRED");
  ...
  async sendPush(lineUserId: string, messages: readonly LineMessage[]) {
    ...
    try {
      response = await fetch("https://api.line.me/v2/bot/message/push", {
        method: "POST",
        headers: { authorization: `Bearer ${this.config.channelAccessToken}`, "content-type": "application/json" },
    ...
    if (!response.ok) throw new LineProviderError("SEND_FAILED");
...
export class FakeLineProvider implements LineProvider {
  ...
  async verifyIdToken(idToken: string) {
    const lineUserId = this.identities.get(idToken);
    if (!lineUserId) throw new LineProviderError("INVALID_TOKEN");
    return { lineUserId };
  }
  async sendPush(lineUserId: string, messages: readonly LineMessage[]) {
    this.sent.push({ lineUserId, messages });

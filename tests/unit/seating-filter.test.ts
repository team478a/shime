...
import { buildSeatingMap, countChangedAssignments, filterSeatingAssignments } from "../../apps/web/src/lib/seating-filter";
...
describe("seating assignment filters", () => {
  it("uses participant-number prefix and partial-name search", () => {
    ...
    expect(filterSeatingAssignments(assignments, participants, "山田", "all").map((row) => row.participantId)).toEqual(["p3"]);
  });

  it("filters assignment state", () => {
    expect(filterSeatingAssignments(assignments, participants, "", "assigned")).toHaveLength(2);
    expect(filterSeatingAssignments(assignments, participants, "", "unassigned").map((row) => row.participantId)).toEqual(["p2"]);
    expect(filterSeatingAssignments(assignments, participants, "", "locked").map((row) => row.participantId)).toEqual(["p3"]);
  });

  it("counts only changed seats and locks", () => {
    expect(countChangedAssignments(assignments, assignments)).toBe(0);
    expect(countChangedAssignments(assignments, [

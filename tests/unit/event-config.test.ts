...
describe("event configuration", () => {
  it("does not hide protected fields", () =>
    expect(() => validateFormFieldRequirement("full_name", "hidden")).toThrow());
  it("requires a visible contact method", () =>
    expect(() =>
      validateContactFields([
        { fieldKey: "phone", requirement: "hidden" },
        { fieldKey: "email", requirement: "hidden" },
      ]),
    ).toThrow());
  it("rejects duplicate seat codes", () =>
    expect(() =>
      validateSeatConfiguration([{ capacity: 2, seats: [{ seatCode: "A1" }, { seatCode: "A1" }] }], 50),
    ).toThrow());
  it("rejects duplicate table codes", () =>
    expect(() =>
      validateSeatConfiguration(
        [
          { tableCode: "T1", capacity: 1, seats: [{ seatCode: "A1" }] },
          { tableCode: "T1", capacity: 1, seats: [{ seatCode: "B1" }] },
        ],
        50,
      ),
    ).toThrow());
  it("warns when seats exceed event capacity", () =>
    expect(
      validateSeatConfiguration([{ capacity: 2, seats: [{ seatCode: "A1" }, { seatCode: "A2" }] }], 1)
...
describe("event configuration completeness", () => {
  ...
  it("lists missing settings without accepting placeholders", () => {
    ...
    expect(result.issues.map((issue) => issue.key)).toEqual(
      expect.arrayContaining(["venueName", "participantCategories"]),
    );
  });
  ...
  it("requires operational resources before accepting applications", () => {
    ...
    expect(result.complete).toBe(false);
    expect(result.issues.map((issue) => issue.key)).toEqual(
      expect.arrayContaining(["eventTables", "eventSeats", "dreamSettings", "questionnaire"]),
    );
  });
  ...
  it("requires published legal documents matching the configured versions", () => {
    const result = includeLegalDocumentReadiness(evaluateEventConfiguration(complete), {
      hasPublishedEventTerms: false,
      hasPublishedPrivacy: true,
    });
    expect(result.complete).toBe(false);
  ...
  it("does not duplicate a legal-document issue when its version is missing", () => {
    const configuration = evaluateEventConfiguration({
      ...complete,
      settings: { ...complete.settings, privacyVersion: "REQUIRED_INPUT" },
    });
    const result = includeLegalDocumentReadiness(configuration, {
      hasPublishedEventTerms: true,
      hasPublishedPrivacy: false,
    });
    expect(
      result.issues.filter((issue) => issue.key === "privacyVersion" || issue.key === "privacyDocument"),
    ).toHaveLength(1);
  });
});

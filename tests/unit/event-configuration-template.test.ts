...
describe("event configuration templates", () => {
  it("validates an application form snapshot", () => {
    expect(
      applicationFormTemplatePayloadSchema.safeParse({
        schemaVersion: 1,
        fields: [
          {
            fieldKey: "phone",
  ...
  it("requires five questionnaire axes and total weight 100", () => {
    const question = (
      axis: "values" | "marriage_intent" | "relationship_pace" | "conversation_style" | "topic_overlap",
      weight: number,
    ) => ({
      axis,
      prompt: "質問",
      ...
      weight,
      options: [
        { code: "a", label: "A", scoreValue: null },
        { code: "b", label: "B", scoreValue: null },
      ],
    });
    const result = questionnaireTemplatePayloadSchema.safeParse({
      schemaVersion: 1,
      code: "standard",
      name: "標準",
      questions: [
        question("values", 20),
        question("marriage_intent", 20),
        question("relationship_pace", 20),
        question("conversation_style", 20),
        question("topic_overlap", 20),
      ],
    });

...
import {
  ...
  getEventAdminNavigation,
  getEventAdminQuickActions,
...
describe("admin navigation", () => {
  ...
  it("builds encoded event-scoped links for managers", () => {
    ...
    expect(groups.flatMap((group) => group.items).find((item) => item.key === "settings")?.href).toBe(
      "/admin/events/event%2F2026/settings",
    );

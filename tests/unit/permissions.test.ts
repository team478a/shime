...
describe("staff permissions", () => {
  it("does not expose preferences to reception", () => expect(hasPermission("reception", "preference:read")).toBe(false));
  ...
  it("keeps unilateral preference exports manager-only", () => { expect(hasPermission("operator", "backup:export")).toBe(true); expect(hasPermission("operator", "backup:sensitive")).toBe(false); expect(hasPermission("manager", "backup:sensitive")).toBe(true); });

...
function seed(scope: number) {
  ...
  return { tenantId, eventId, statements };
}

describe("operations analytics repository scope", () => {
  it("aggregates only the requested tenant/event without returning private content or identifiers", async () => {
    client = new PGlite();
    await migrate(drizzle(client), { migrationsFolder: "packages/db/migrations" });
    const requested = seed(1);
    const noise = seed(2);
    await client.exec([...requested.statements, ...noise.statements].join(";\n"));

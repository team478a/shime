...
test("LINE Webhookはtenant指定のないリクエストを拒否する", async ({ request }) => {
  const response = await request.post("/api/webhooks/line", {
    data: { events: [] },
  });
  expect(response.status()).toBe(400);
  expect(await response.json()).toEqual({ code: "TENANT_REQUIRED" });
});

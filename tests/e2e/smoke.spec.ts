...
test("検証環境バナーと公開トップを表示する", async ({ page }) => { await page.goto("/"); await expect(page).toHaveTitle(/SHIME/); await expect(page.getByText(/検証環境（test）/)).toBeVisible(); });
test("未認証の管理画面アクセスをログインへ戻す", async ({ page }) => { await page.goto("/admin"); await expect(page).toHaveURL(/\/admin\/login$/); await expect(page.getByRole("heading", { name: "SHIME 運営ログイン" })).toBeVisible(); });
test("未認証の管理APIをrequest ID付きで拒否する", async ({ request }) => { const response = await request.get("/api/admin/events"); expect(response.status()).toBe(401); expect(response.headers()["x-request-id"]).toMatch(/^[0-9a-f-]{36}$/); });
...
test("Dream回答保存後に候補生成だけを再試行できる", async ({ page }) => {
  ...
  await page.route("**/api/liff/events/event-1", async (route) => route.fulfill({
    status: 200,
    contentType: "application/json",
    body: JSON.stringify({ data: { name: "テスト交流会", statusLabel: "イベント進行中", startsAt: "2026-08-08T05:00:00.000Z", endsAt: null, venueName: "テスト会場", venueAddress: null } }),
  }));
  await page.route("**/api/liff/events/event-1/emotion-cards", async (route) => route.fulfill({
    status: 200,
    contentType: "application/json",
    body: JSON.stringify({ data: { registrationMode: "optional", cards: [{ id: "11111111-1111-4111-8111-111111111111", name: "希望", imageKey: null, description: "明るい未来" }] } }),
  }));
  await page.route("**/api/liff/events/event-1/emotion-selection", async (route) => route.fulfill({
    status: 200,
    contentType: "application/json",
  ...
  await page.route("**/api/liff/events/event-1/dream/suggestions", async (route) => {
    ...
    await route.fulfill(suggestionRequests === 1 ? {
      status: 503,
      contentType: "application/json",
      ...
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ data: { candidates: ["穏やかな家庭を育てる", "新しい一歩を踏み出す", "安心できる関係を築く"] } }),
    });
  });

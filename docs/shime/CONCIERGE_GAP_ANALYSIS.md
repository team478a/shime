...
1. 既存Dream用の感情カード・回答と、新規診断用カード・回答を物理的にも意味的にも分離する。
...
- `emotion_card_sets`、`emotion_cards`、`emotion_selections`はDream専用である。診断用に意味を拡張しない。
...
| C-02 | Dreamの既存emotion tablesを変更せず、診断用card set/card/mappingを別テーブルにする。 | 設計確定候補 |

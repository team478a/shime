...
最終更新: 2026-08-07 12:32（Asia/Tokyo、Codex。production migration 0024適用・事後検証完了、未デプロイ）
...
production deployment source HEAD: `9b4e42e1c82ac97819d1bda4b8b2f7cc7344e47a`
...
### production migration 0024適用（2026-08-07 12:32 JST、デプロイ・機能ONは未実施）
...
- 適用後も`event_match_chat_configs`、`match_chat_rooms`、`match_chat_messages`はすべて0件で、マッチ後チャットはOFF。デプロイ、機能ON、実データ操作、LINE通知は実施していない。

### マッチ後チャット有効化ゲート強化（2026-08-07、migration適用済み、未デプロイ）
...
- migration 0024はproduction適用済みで事後検証も成功。production deploy、機能ON、実データ操作、LINE通知は行っていない。次はデプロイ前確認と、別途承認後のrelease `af6f501`本番デプロイ。正式規約本文、保持日数、通報対応責任者、合成データUATが確定・完了するまで機能をONにしない。

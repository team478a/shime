export const QUESTIONNAIRE_AXES = ["values", "marriage_intent", "relationship_pace", "conversation_style", "topic_overlap"] as const;
...
export type QuestionnaireQuestionDraft = { axis: QuestionnaireAxis; prompt: string; kind: QuestionnaireKind; maxSelections: number; weight: number; options: QuestionnaireOptionDraft[] };
...
export function questionnaireEditorIssues(questions: QuestionnaireQuestionDraft[]) {
  const issues: string[] = [];
  if (questions.length !== 5 || new Set(questions.map((question) => question.axis)).size !== 5) issues.push("5つの軸を1問ずつ設定してください。");
  ...
  questions.forEach((question, index) => {
    ...
    if (codes.length !== question.options.length || question.options.some((option) => !option.label.trim())) issues.push(`${label}の選択肢コードと表示名を入力してください。`);
    if (new Set(codes).size !== codes.length) issues.push(`${label}の選択肢コードが重複しています。`);
    if (question.maxSelections < 1 || question.maxSelections > question.options.length) issues.push(`${label}の最大選択数を選択肢数以下にしてください。`);
  });

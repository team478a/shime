...
export function buildLinePublicUrls(liffId: string, appUrl: string | null | undefined, tenantCode?: string) {
  const origin = appUrl?.replace(/\/$/, "") ?? null;
  return {
    liffUrl: liffId ? `https://liff.line.me/${liffId}` : null,
    endpointUrl: origin ? `${origin}/liff/link` : null,
    webhookUrl: origin && tenantCode ? `${origin}/api/webhooks/line?tenant=${encodeURIComponent(tenantCode)}` : null,
  };
}

export function parseLiffLinkQuery(query: Record<string, string | string[] | undefined>) {
  const directEventId = typeof query.eventId === "string" ? query.eventId : "";
  const directLinkToken = typeof query.linkToken === "string" ? query.linkToken : "";
  ...
  const stateQuery = questionMark >= 0 ? state.slice(questionMark + 1) : state.startsWith("?") ? state.slice(1) : state;

...
export default async function LegalDocumentPage({
  ...
  params: Promise<{ eventId: string; documentType: string }>;
  searchParams: Promise<{ version?: string }>;
}) {
  const { eventId, documentType } = await params;
  const { version } = await searchParams;
  if (!isLegalDocumentType(documentType)) notFound();
  const db = getDatabase();
  const event = (
    await db
      .select({ id: events.id, tenantId: events.tenantId, name: events.name })
      .from(events)
  ...
  if (version) conditions.push(eq(legalDocuments.version, version));
  const document = (
    await db
      .select()
      .from(legalDocuments)
  ...
  const publishedAt = document.publishedAt?.toLocaleString("ja-JP", {
    timeZone: "Asia/Tokyo",
    dateStyle: "long",
    timeStyle: "short",
  });

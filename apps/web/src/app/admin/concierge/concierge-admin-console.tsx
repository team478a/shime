...
function rowsFromPayload(payload: ConciergeTemplatePayload) {
  ...
  payload.emotions.forEach((emotion, index) => {
    if (emotions[index])
      emotions[index] = { code: emotion.code, label: emotion.label, description: emotion.description };
  });
  return {
    questions,
    emotions,
    mappings: payload.cardMappings.map((mapping) => ({
      cardAssetVersionId: mapping.cardAssetVersionId,
      emotionCode: mapping.emotionCode,
    })),
  };
}

export function ConciergeAdminConsole({
  initialTemplates,
  initialCards,
  canPublish,
}: {
    ...
    () =>
      cards.flatMap((card) =>
        card.versions
          ...
          .map((version) => ({ ...version, assetName: card.name, assetCode: card.code })),
      ),
    [cards],
  );
  ...
  function buildPayload(): ConciergeTemplatePayload {
    return {
      ...
      emotions: emotions.flatMap((emotion, index) =>
        !emotion.code.trim() && !emotion.label.trim()
          ? []
          : [
              {
      ...
      cardMappings: mappings
        .filter((mapping) => mapping.cardAssetVersionId && mapping.emotionCode.trim())
        .map((mapping, index) => ({
          cardAssetVersionId: mapping.cardAssetVersionId,
  ...
  async function saveTemplate() {
    ...
    const selectedDraft = templates
      .find((template) => template.id === templateId)
      ?.versions.find((version) => version.status === "draft");
    const response = selectedDraft
      ? await fetch(`/api/admin/concierge/templates/versions/${selectedDraft.id}`, {
          method: "PATCH",
          headers: { "content-type": "application/json" },
          body: JSON.stringify(draft),
        })
      : await fetch("/api/admin/concierge/templates", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ name: templateName, templateId: templateId || undefined, payload: draft }),
        });
    const result = await response.json().catch(() => ({}));
    setBusy(false);
    if (!response.ok) {
      setMessage(
        `保存できませんでした（${result.code ?? response.status}）。入力コードは半角小文字・数字・_を使用してください。`,
      );
      return;
    }
    ...
    if (!response.ok) {
      setMessage(
        result.issues?.map((issue: { message: string }) => issue.message).join(" ") ??
          `公開できませんでした（${result.code ?? response.status}）。`,
      );
      return;
    }
    setMessage("公開しました。");
  ...
  async function uploadCard(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    setMessage("");
    const response = await fetch("/api/admin/concierge/cards", {
      method: "POST",
    ...
    if (!response.ok) {
      setMessage(
        `画像を保存できませんでした（${result.code ?? response.status}）。JPEG/PNG/WebP、5MB以下、512px以上を確認してください。`,
      );
      return;
    }
  ...
  return (
    <div className="admin-stack">
      <section className="panel wide">
        ...
        <h1>診断・カード管理</h1>
        <p>
          現在は設定・版管理のみです。外部AI接続と参加者向け診断はまだ有効化されません。正式文言が未確定でも下書き保存できます。
        </p>
        {message && (
      ...
      <section className="panel wide">
        ...
        <div className="settings-grid">
          <label>
            画面タイトル
            <input
              value={payload.copy.pageTitle}
              onChange={(event) =>
                setPayload((current) => ({ ...current, copy: { ...current.copy, pageTitle: event.target.value } }))
              }
            />
          </label>
          <label>
            完了見出し
            <input
              value={payload.copy.completionTitle}
              onChange={(event) =>
                setPayload((current) => ({
                  ...current,
                  copy: { ...current.copy, completionTitle: event.target.value },
                }))
              }
            />
          </label>
        ...
        <label>
          ...
          <textarea
            rows={3}
            value={payload.copy.intro}
            onChange={(event) =>
              setPayload((current) => ({ ...current, copy: { ...current.copy, intro: event.target.value } }))
            }
          />
        </label>
        <label>
          ...
          <textarea
            rows={3}
            value={payload.copy.instructions}
            onChange={(event) =>
              setPayload((current) => ({ ...current, copy: { ...current.copy, instructions: event.target.value } }))
            }
          />
        </label>
        <label>
          ...
          <textarea
            rows={3}
            value={payload.copy.completionBody}
            onChange={(event) =>
              setPayload((current) => ({ ...current, copy: { ...current.copy, completionBody: event.target.value } }))
            }
          />
        </label>
        <div className="settings-grid">
          {(["startButton", "nextButton", "backButton", "completeButton"] as const).map((key) => (
            <label key={key}>
              {{ startButton: "開始ボタン", nextButton: "次へ", backButton: "戻る", completeButton: "完了ボタン" }[key]}
              <input
                value={payload.copy[key]}
                onChange={(event) =>
                  setPayload((current) => ({ ...current, copy: { ...current.copy, [key]: event.target.value } }))
                }
              />
            </label>
          ))}
        ...
        <fieldset>
          ...
          {(
            ...
            <label key={key}>
              <input
                type="checkbox"
                checked={payload.protectedMessageKeys.includes(key)}
                onChange={(event) =>
                  setPayload((current) => ({
                    ...current,
          ...
          <fieldset key={index}>
            ...
            <div className="settings-grid">
              <label>
                軸コード
                <input
                  value={question.axisCode}
                  placeholder="axis_code"
                  onChange={(event) =>
                    setQuestions((current) =>
                      current.map((row, rowIndex) =>
                        rowIndex === index ? { ...row, axisCode: event.target.value } : row,
                      ),
                    )
                  }
                />
            ...
            <label>
              補足文
              <input
                value={question.supplementalText}
                onChange={(event) =>
                  setQuestions((current) =>
                    current.map((row, rowIndex) =>
                      rowIndex === index ? { ...row, supplementalText: event.target.value } : row,
                    ),
                  )
                }
              />
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setQuestions((current) =>
                    current.map((row, rowIndex) =>
                      rowIndex === index ? { ...row, options: event.target.value } : row,
                    ),
                  )
                }
              />
          ...
          <fieldset key={index}>
            ...
            <div className="settings-grid">
              <label>
                ...
                <input
                  value={emotion.code}
                  placeholder="emotion_code"
                  onChange={(event) =>
                    setEmotions((current) =>
                      current.map((row, rowIndex) => (rowIndex === index ? { ...row, code: event.target.value } : row)),
                    )
                  }
                />
              </label>
        ...
        {mappings.map((mapping, index) => (
          <div className="settings-grid" key={index}>
            <label>
              公開済みカード
              <select
                value={mapping.cardAssetVersionId}
                onChange={(event) =>
                  setMappings((current) =>
                    current.map((row, rowIndex) =>
                      rowIndex === index ? { ...row, cardAssetVersionId: event.target.value } : row,
                    ),
                  )
                }
              >
                ...
                {publishedCards.map((card) => (
                  <option key={card.id} value={card.id}>
                    {card.assetCode} / {card.title} v{card.version}
                  </option>
                ))}
              </select>
            </label>
            <label>
              感情コード
              <input
                value={mapping.emotionCode}
                onChange={(event) =>
                  setMappings((current) =>
                    current.map((row, rowIndex) =>
                      rowIndex === index ? { ...row, emotionCode: event.target.value } : row,
                    ),
                  )
                }
              />
            ...
            <div className="actions">
              <button type="button" className="secondary" disabled={index === 0} onClick={() => moveMapping(index, -1)}>
                上へ
              </button>
              <button
                type="button"
              ...
              <button
                type="button"
                className="secondary"
                onClick={() => setMappings((current) => current.filter((_, rowIndex) => rowIndex !== index))}
              >
                割当を削除
              </button>
            </div>
        ...
        <button
          type="button"
          className="secondary"
          onClick={() => setMappings((current) => [...current, { cardAssetVersionId: "", emotionCode: "" }])}
        >
          カード割当を追加
        </button>
        ...
        <div className="settings-grid">
          <label>
            タイトル
            <input
              value={payload.reportCopy.title}
              onChange={(event) =>
                setPayload((current) => ({
                  ...current,
                  reportCopy: { ...current.reportCopy, title: event.target.value },
                }))
              }
            />
          </label>
          <label>
            見出し
            <input
              value={payload.reportCopy.heading}
              onChange={(event) =>
                setPayload((current) => ({
                  ...current,
                  reportCopy: { ...current.reportCopy, heading: event.target.value },
                }))
              }
            />
          </label>
        </div>
        {(["fixedText", "disclaimer", "guidance"] as const).map((key) => (
          <label key={key}>
            {{ fixedText: "固定文", disclaimer: "免責文", guidance: "案内文" }[key]}
            <textarea
              rows={3}
              value={payload.reportCopy[key]}
              onChange={(event) =>
                setPayload((current) => ({
                  ...current,
                  reportCopy: { ...current.reportCopy, [key]: event.target.value },
                }))
              }
            />
          </label>
      ...
      <section className="panel wide">
        ...
        {templates.map((template) => (
          <article className="admin-list-card" key={template.id}>
            <h3>{template.name}</h3>
            {template.versions.map((version) => (
              <div className="actions" key={version.id}>
                ...
                {version.status === "draft" && canPublish && (
                  <button type="button" disabled={busy} onClick={() => publish("templates", version.id)}>
                    公開
                  </button>
                )}
              </div>
      ...
      <section className="panel wide">
        ...
        <form className="settings-form" onSubmit={uploadCard}>
          ...
          <div className="settings-grid">
            <label>
              カードコード
              <input name="code" required pattern="[a-z0-9_]{2,80}" />
            </label>
            <label>
              管理名
          ...
          <label>
            画像の代替テキスト
            <input name="altText" required maxLength={500} />
          </label>
          <label>
            画像（JPEG / PNG / WebP、5MB以下、512×512px以上）
        ...
        {cards.map((card) => (
          <article className="admin-list-card" key={card.id}>
            ...
            {card.versions.map((version) => (
              <div key={version.id}>
                <p>
                  v{version.version} / {version.status} / {version.title} / {version.width}×{version.height}px
                </p>
                <div className="actions">
                  <a
                    className="button-link secondary"
                    href={`/api/admin/concierge/cards/versions/${version.id}/image`}
                    target="_blank"
                  >
                    画像確認
                  </a>
                  {version.status === "draft" && canPublish && (
                    <button type="button" disabled={busy} onClick={() => publish("cards", version.id)}>
                      公開
                    </button>
                  )}
                </div>

...
import { conciergeCardAssets, conciergeCardAssetVersions, conciergeTemplates, conciergeTemplateVersions, getDatabase } from "@shime/db";
...
export default async function ConciergeAdminPage() {
  ...
  const [templateRows, versionRows, assetRows, cardVersionRows] = await Promise.all([
    db.select().from(conciergeTemplates).where(and(
    ...
    db.select().from(conciergeTemplateVersions).where(eq(conciergeTemplateVersions.tenantId, session.tenantId)).orderBy(desc(conciergeTemplateVersions.version)),
    db.select().from(conciergeCardAssets).where(and(
    ...
    db.select().from(conciergeCardAssetVersions).where(eq(conciergeCardAssetVersions.tenantId, session.tenantId)).orderBy(desc(conciergeCardAssetVersions.version)),
  ]);
  const templates: ConciergeAdminTemplate[] = templateRows.map((template) => ({
    id: template.id,
    name: template.name,
    versions: versionRows.filter((version) => version.templateId === template.id).flatMap((version) => {
  ...
  const cards: ConciergeAdminCard[] = assetRows.map((asset) => ({
    ...
    code: asset.code,
    name: asset.name,
    versions: cardVersionRows.filter((version) => version.assetId === asset.id).map((version) => ({
      id: version.id,
      version: version.version,
      status: version.status,
      title: version.title,
  ...
  return <main><ConciergeAdminConsole initialTemplates={templates} initialCards={cards} canPublish={hasPermission(session.role, "concierge:publish")} /></main>;
}

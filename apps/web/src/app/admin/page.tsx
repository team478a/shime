...
export default async function AdminPage() {
  ...
  return (
    <main>
      <section className="panel admin-panel admin-dashboard">
        <p className="eyebrow">ADMIN DASHBOARD</p>
        <h1>{session.displayName}さん</h1>
        <p>担当するイベントと、現在の設定状況を確認できます。</p>
        ...
        <details className="admin-resource-downloads">
          ...
          <div className="actions">
            {getAdminPublicDownloads().map((document) => (
              <a
                key={document.outputName}
                className="button-link secondary"
                href={`/downloads/${document.outputName}`}
                download
              >
                {document.label}
              </a>
        ...
        <div className="actions">
          <h2>イベント</h2>
          {canWriteEvent && (
            <Link className="button-link" href="/admin/events/new">
              イベントを作成
            </Link>
          )}
        </div>
        {eventItems.length ? (
          <ul className="event-list admin-event-list">
            {eventItems.map(
              ({ event, configuration, href, analyticsHref, checkinHref, participantsHref, seatingHref }) => (
                <li key={event.id}>
                  <div>
                    <strong>{event.name}</strong>
                    <small>
                      {getEventStatusLabel(event.status)} /{" "}
                      {configuration.complete ? "必須設定入力済み" : `未設定 ${configuration.issues.length}件`}
                    </small>
                  </div>
                  <div className="event-quick-actions">
                    {analyticsHref && (
                      <Link className="button-link" href={analyticsHref}>
                        運営進捗
                      </Link>
                    )}
                    {checkinHref && (
                      <Link className="button-link event-checkin-link" href={checkinHref}>
                        当日受付
                      </Link>
                    )}
                    {seatingHref && (
                      <Link className="button-link" href={seatingHref}>
                        席配置
                      </Link>
                    )}
                    {participantsHref && (
                      <Link className="button-link secondary" href={participantsHref}>
                        参加者・LINE連携
                      </Link>
                    )}
                    {href && (
                      <Link className="button-link secondary" href={href}>
                        設定・詳細
                      </Link>
                    )}
                  </div>
          ...
          <p>イベントがありません。責任者またはシステム管理者が下書きイベントを作成してください。</p>
        )}
      </section>
    </main>
  );

...
export default async function VenueLayoutTemplatesPage() {
  ...
  const applications = templateIds.length
    ? await db
        .select()
        .from(resourceTemplateApplications)
        .where(
          and(
    ...
    : [];
  const eventIds = [
    ...new Set(applications.filter((item) => item.targetType === "event").map((item) => item.targetId)),
  ];
  const eventRows = eventIds.length
    ? await db
        .select({ id: events.id, name: events.name })
        .from(events)
        .where(and(eq(events.tenantId, session.tenantId), inArray(events.id, eventIds)))
    : [];
  ...
  for (const template of templates) {
    ...
    const version = {
      ...
      ...(lastUse
        ? {
            lastUsed: {
              eventName: eventNames.get(lastUse.targetId) ?? "削除済みイベント",
              appliedAt: formatDate(lastUse.appliedAt),
            },
          }
        : {}),
      preview: [...payload.data.tables]
        .sort((left, right) => left.displayOrder - right.displayOrder)
        .map((table) => ({ tableCode: table.tableCode, seatCodes: table.seats.map((seat) => seat.seatCode) })),
    };
  ...
  return (
    <main>
      <VenueLayoutTemplateManager groups={[...grouped.values()]} />
    </main>
  );
}

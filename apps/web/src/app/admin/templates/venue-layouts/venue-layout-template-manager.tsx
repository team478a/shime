...
export function VenueLayoutTemplateManager({ groups }: Readonly<{ groups: VenueLayoutTemplateGroup[] }>) {
  ...
  async function archive() {
    ...
    try {
      ...
      if (!response.ok) {
        setError(true);
        setMessage(
          result?.code === "ALREADY_ARCHIVED" ? "この版は既にアーカイブされています。" : "アーカイブできませんでした。",
        );
        return;
      }
      setArchiveId("");
  ...
  async function restoreAsNewVersion(templateId: string, name: string, currentVersion: number) {
    if (
      !window.confirm(
        `「${name}」v${currentVersion}の内容を引き継ぎ、次の版を利用中として作成しますか？旧版は履歴のまま保持されます。`,
      )
    )
      return;
    ...
    try {
      ...
      if (!response.ok || !result?.data) {
        setError(true);
        setMessage(
          result?.code === "SOURCE_VERSION_STILL_ACTIVE"
            ? "この版は既に利用中です。"
            : "新しい版を作成できませんでした。",
        );
        return;
  ...
  return (
    <div className="admin-stack venue-template-page">
      <section className="panel wide">
        <p className="eyebrow">REUSABLE VENUE LAYOUTS</p>
        <h1>会場テンプレート管理</h1>
        <p>
          会場ごとのテーブル・席構成を版付きで管理します。過去の版とイベントへ適用した時点の構成は削除・上書きされません。
        </p>
        <div className="summary">
          <span>テンプレート {groups.length}件</span>
          <span>有効 {groups.filter((group) => group.versions.some((version) => version.active)).length}件</span>
        </div>
        {!groups.length && (
          <p className="participant-empty">
            会場テンプレートはまだありません。イベントの「テーブル・席マスター」で現在の構成を保存できます。
          </p>
        )}
      </section>
      ...
      {groups.map((group) => {
        ...
        return (
          <section className="panel wide venue-template-card" key={group.key}>
            <div className="venue-template-heading">
              ...
              <span
                className={
                  group.versions.some((version) => version.active) ? "status-badge status-complete" : "status-badge"
                }
              >
                {group.versions.some((version) => version.active) ? "利用中" : "アーカイブ済み"}
              </span>
            </div>
            {latest && (
              <div className="summary">
                <span>最新 v{latest.version}</span>
                <span>{latest.tableCount}卓</span>
                <span>{latest.seatCount}席</span>
                <span>適用 {group.versions.reduce((total, version) => total + version.usageCount, 0)}回</span>
              </div>
            )}
            <div className="table-layout-preview-grid venue-template-preview">
              {latest?.preview.map((table) => (
                <article className="table-layout-preview-card" key={table.tableCode}>
                  <div className="table-layout-preview-label">
                    <small>TABLE</small>
                    <strong>{table.tableCode}</strong>
                  </div>
                  <div className="table-layout-seat-chips">
                    {table.seatCodes.map((seat) => (
            ...
            <details className="table-setup-tool">
              ...
              <div className="table-setup-tool-body venue-template-version-list">
                {group.versions.map((version) => (
                  <article key={version.id} className="venue-template-version">
                    <div>
                      <strong>v{version.version}</strong> <span>{version.active ? "利用中" : "履歴"}</span>
                    </div>
                    <p>
                      {version.tableCount}卓・{version.seatCount}席／作成 {version.createdAt}
                    </p>
                    <p>
                      イベント適用 {version.usageCount}回
                      {version.lastUsed ? `／最終 ${version.lastUsed.eventName}（${version.lastUsed.appliedAt}）` : ""}
                    </p>
                    {version.active && (
                      <button
                        type="button"
                    ...
                    {!version.active && version.id === latest?.id && (
                      <button
                        type="button"
                        className="secondary"
                        disabled={busy}
                        onClick={() => restoreAsNewVersion(version.id, group.name, version.version)}
                      >
                        {restoreId === version.id ? "作成中…" : "この版から新しい版を作成"}
                      </button>
                    )}
                  </article>
                ))}
      ...
      {archiveId && (
        <section className="panel wide venue-template-archive" aria-label="テンプレートのアーカイブ確認">
          <h2>アーカイブ理由</h2>
          ...
          <label>
            ...
            <select
              value={reason}
              onChange={(event) => setReason(event.target.value as (typeof reasonOptions)[number])}
            >
          ...
          <div className="actions">
            ...
            <button type="button" className="secondary" disabled={busy} onClick={() => setArchiveId("")}>
              やめる
            </button>
          </div>
        </section>

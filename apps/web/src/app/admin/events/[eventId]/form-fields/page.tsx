...
export default async function FormFieldsPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const templates = templateRows.flatMap((template) => {
    const payload = applicationFormTemplatePayloadSchema.safeParse(template.payload);
      ...
      ? [
          {
            ...
            rows: payload.data.fields.map((field) => ({
              fieldKey: field.fieldKey,
              label: field.label,
              type: field.type,
              requirement: field.requirement,
              displayOrder: field.displayOrder,
              options: Array.isArray(field.validation.options)
                ? field.validation.options.filter((option): option is string => typeof option === "string").join(", ")
                : "",
            })),
          },
        ]
    ...
    <main>
      <FormFieldSettings
        ...
        initial={fields.map((field) => ({
          ...
          requirement: field.requirement,
          displayOrder: field.displayOrder,
          options: Array.isArray(field.validation.options) ? field.validation.options.join(", ") : "",
        }))}
        templates={templates}

  ...
  return (
    <form onSubmit={submit} className="admin-stack">
      <section className="panel wide">
        ...
        <label>
          夢登録方式
          <select name="registrationMode" defaultValue={initial.registrationMode}>
            <option value="required_private_allowed">必須（非公開可）</option>
            <option value="optional">任意（スキップ可）</option>
          </select>
        </label>
        <label>
          <input name="aiEnabled" type="checkbox" defaultChecked={initial.aiEnabled} /> AI文章生成を有効化する
        </label>
        <p className="hint">
          外部接続・運用設定でOpenAIを有効化してください。接続失敗やタイムアウト時は固定文へ自動で戻ります。
        </p>
        <label>
          AIタイムアウト（ms）
          <input name="aiTimeoutMs" type="number" min="1000" max="10000" defaultValue={initial.aiTimeoutMs} />
        </label>
        <label>
          プロジェクト同意文書版
          <input name="projectConsentVersion" defaultValue={initial.projectConsentVersion} />
        </label>
      </section>
      <section className="panel wide">
        <h2>固定文フォールバック</h2>
        <label>
          橋渡し文テンプレート
          <textarea name="fallbackBridgeTemplate" defaultValue={initial.fallbackBridgeTemplate} />
          <small>{"{card} と {wish} を利用できます"}</small>
        </label>
        {initial.fallbackCandidates.map((candidate, index) => (
          <label key={index}>
            夢候補 {index + 1}
            <input name={`candidate${index + 1}`} defaultValue={candidate} />
          </label>
        ))}
      </section>
      <section className="panel wide">
        ...
        <label>
          コード
          <input name="cardSetCode" defaultValue={initial.cardSetCode} required />
        </label>
        <label>
          名称
          <input name="cardSetName" defaultValue={initial.cardSetName} required />
        </label>
        <label>
          バージョン
          <input name="cardSetVersion" type="number" min="1" defaultValue={initial.cardSetVersion} />
        </label>
        <label>
          カード（1行1枚：名称 | 画像キー | 説明）
          <textarea name="cards" rows={10} defaultValue={initial.cardsText} required />
        </label>

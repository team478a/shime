...
export default async function LegalDocumentsPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!event) notFound();
  const documents = await db.select().from(legalDocuments).where(and(eq(legalDocuments.tenantId, session.tenantId), eq(legalDocuments.eventId, eventId))).orderBy(desc(legalDocuments.createdAt));
  return <main><LegalDocumentConsole key={documents.map((document) => `${document.id}:${document.updatedAt.toISOString()}`).join("|")} eventId={eventId} eventName={event.name} initial={documents.map((document) => ({ ...document, publishedAt: document.publishedAt?.toISOString() ?? null, createdAt: document.createdAt.toISOString(), updatedAt: document.updatedAt.toISOString() }))} /></main>;
}

...
function responseMessage(code?: string): string {
  if (code === "SEAT_CONFIGURATION_IN_USE")
    return "配置案が作成済みのため変更できません。配置案を作る前にテーブル設定を確定してください。";
...
export function TableSettingsForm({
  ...
  const [rows, setRows] = useState<TableLayoutRow[]>(
    initial.length ? initial : [{ tableCode: "T01", capacity: 2, seats: "T01-1, T01-2" }],
  );
  ...
  function replaceRows(next: TableLayoutRow[], source: string, templateId?: string) {
    if (
      rows.some((row) => row.tableCode || row.seats) &&
      !window.confirm(`現在の入力内容を${source}の内容で置き換えますか？まだ保存はされません。`)
    )
      return false;
    setRows(next);
  ...
  function applySource() {
    ...
    if (replaceRows(source.rows, source.label, source.templateId))
      setTemplateName(templateNameAfterSourceCopy(templateName, source));
  }
  ...
  async function saveTemplate() {
    ...
    if (
      !window.confirm(
        `現在の${validation.tableCount}卓・${validation.seatCount}席を「${name}」としてテナント共通テンプレートに保存しますか？同名の場合は新しい版になります。`,
      )
    )
      return;
    setBusy("template");
    ...
    try {
      ...
      if (latestResponse.ok && Array.isArray(latest?.data)) {
        const templateSources: TableLayoutSource[] = latest.data.map(
          (item: {
            id: string;
            name: string;
            version: number;
            ...
            rows: [...item.payload.tables]
              .sort((left, right) => left.displayOrder - right.displayOrder)
              .map((table) => ({
                tableCode: table.tableCode,
                capacity: table.capacity,
                seats: table.seats.map((seat) => seat.seatCode).join(", "),
              })),
          }),
        );
  ...
  async function save() {
    ...
    if (
      !window.confirm(
        `${validation.tableCount}卓・${validation.seatCount}席を保存します。既存のテーブル設定は置き換わります。${warningText}`,
      )
    )
      return;
    setBusy("event");
  ...
  return (
    <div className="admin-stack table-setup-page">
      <section className="panel wide">
        ...
        <div className="summary table-layout-summary" aria-label="テーブル設定件数">
          <span>イベント定員 {eventCapacity}名</span>
          <span>テーブル {validation.tableCount}卓</span>
          <span>席 {validation.seatCount}席</span>
        </div>
        ...
        <details className="table-setup-tool" open={sources.length > 0}>
          ...
          <div className="table-setup-tool-body">
            <p className="hint">
              コピー後にこのイベント用として調整できます。コピー元のテンプレートや過去イベントは変更されません。
            </p>
            {sourceTemplateId && (
              <p className="operation-feedback">
                会場テンプレートを元に編集中です。イベントへ保存した時点の最終構成と元版を履歴に残します。
              </p>
            )}
            {sources.length ? (
              <div className="table-layout-source-controls">
                <label>
                  コピー元
                  <select value={selectedSourceId} onChange={(event) => setSelectedSourceId(event.target.value)}>
                    <option value="">選択してください</option>
                    <optgroup label="会場テンプレート">
                      {sources
                ...
                <button
                  type="button"
                  className="secondary"
                  disabled={!selectedSourceId || Boolean(busy)}
                  onClick={applySource}
                >
                  選択した構成をコピー
                </button>
              ...
              <p className="participant-empty">利用できるテンプレートまたは過去イベントはまだありません。</p>
            )}
            {canCreateTemplate && (
              <div className="table-template-save">
                <label>
                  テンプレート名
                  <input
                    value={templateName}
                    ...
                    placeholder="例: 市民ホールA 50名配置"
                  />
                </label>
                <button
                  type="button"
                  className="secondary"
                  disabled={Boolean(busy) || validation.errors.length > 0}
                  onClick={saveTemplate}
                >
                  {busy === "template" ? "保存中…" : "現在の構成をテンプレート保存"}
                </button>
              </div>
            )}
          </div>
        ...
        <details className="table-setup-tool" open={!initial.length}>
          ...
          <div className="table-setup-tool-body">
            ...
            <div className="settings-grid table-generator-grid">
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) => setFirstTableNumber(Number(event.target.value))}
                />
              </label>
              <label>
                テーブル数
            ...
            <button type="button" className="secondary" onClick={applyGenerator}>
              この条件で座席を作成
            </button>
          </div>
        </details>
        ...
        <details className="table-setup-tool">
          ...
          <div className="table-setup-tool-body">
            ...
            <div className="actions">
              <a className="button-link secondary" href="/templates/TABLE_IMPORT_TEMPLATE.csv" download>
                CSVひな形を取得
              </a>
              <label className="button-link secondary table-csv-file">
                CSVファイルを選択
        ...
        {(validation.errors.length > 0 || validation.warnings.length > 0) && (
          <section
            className={validation.errors.length ? "configuration-incomplete" : "table-layout-warning"}
            aria-label="保存前の確認事項"
          >
            <h2>保存前の確認</h2>
            <ul>
        ...
        <section className="table-layout-preview" aria-label="テーブル配置プレビュー">
          <div>
            <h2>座席図プレビュー</h2>
            <p className="hint">実際の会場内の位置ではなく、各テーブルの席構成を表します。</p>
          </div>
          <div className="table-layout-preview-grid">
            {rows.map((row, index) => (
              <article className="table-layout-preview-card" key={`${row.tableCode}-${index}`}>
                <div className="table-layout-preview-label">
                  <small>TABLE</small>
                  <strong>{row.tableCode || `未入力 ${index + 1}`}</strong>
                </div>
                <div className="table-layout-seat-chips">
                  {splitSeatCodes(row.seats).map((seatCode) => (
                    <span key={seatCode}>{seatCode}</span>
                  ))}
                </div>
                <small>
                  {splitSeatCodes(row.seats).length}席 / 定員{row.capacity}名
                </small>
              </article>
            ))}
          </div>
        ...
        <details className="table-manual-editor" open>
          ...
          <div className="table-manual-editor-body">
            {rows.map((row, index) => (
              <fieldset key={index}>
                ...
                <div className="settings-grid">
                  <label>
                    ...
                    <input
                      value={row.tableCode}
                      onChange={(event) => update(index, { tableCode: event.target.value })}
                    />
                  </label>
                  <label>
                    定員
        ...
        <div className="actions table-settings-actions">
          <button
            ...
            onClick={() =>
              setRows((current) => [
                ...current,
                { tableCode: `T${String(current.length + 1).padStart(2, "0")}`, capacity: 2, seats: "" },
              ])
            }
          >
            テーブルを追加
          </button>
          <button type="button" disabled={Boolean(busy) || validation.errors.length > 0} onClick={save}>
            {busy === "event" ? "保存中…" : `テーブル・席を保存（${validation.seatCount}席）`}
          </button>
          <a className="button-link secondary" href={`/admin/events/${eventId}/setup`}>
            設定チェックへ戻る
          </a>
        </div>
        {message && (

...
export default async function TablesPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const event = (
    await db
      .select({ capacity: events.capacity, venueName: events.venueName })
      .from(events)
  ...
  const tables = await db
    ...
    .orderBy(asc(eventTables.displayOrder));
  ...
  const priorEvents = session.eventId
    ? []
    : await db
        .select({ id: events.id, name: events.name, venueName: events.venueName, startsAt: events.startsAt })
        .from(events)
        .where(and(eq(events.tenantId, session.tenantId), ne(events.id, eventId)))
        .orderBy(desc(events.startsAt))
        .limit(20);
  const priorEventIds = priorEvents.map((item) => item.id);
  const priorTables = priorEventIds.length
    ? await db
        .select()
        .from(eventTables)
        .where(and(eq(eventTables.tenantId, session.tenantId), inArray(eventTables.eventId, priorEventIds)))
        .orderBy(asc(eventTables.displayOrder))
    : [];
  const priorSeats = priorEventIds.length
    ? await db
        .select()
        .from(eventSeats)
        .where(and(eq(eventSeats.tenantId, session.tenantId), inArray(eventSeats.eventId, priorEventIds)))
    : [];
    ...
    return [
      {
        ...
        rows: sourceTables.map((table) => ({
          ...
          capacity: table.capacity,
          seats: priorSeats
            .filter((seat) => seat.eventId === sourceEvent.id && seat.tableId === table.id)
            .map((seat) => seat.seatCode)
            .sort(codeCollator.compare)
            .join(", "),
        })),

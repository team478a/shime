...
import {
  filterParticipantLinkRows,
  type ParticipantLinkFilter,
} from "../../../../../lib/participant-link-filter";
...
export function ParticipantLinkConsole({
  ...
  async function reissue(participant: ParticipantRow) {
    if (!confirm(`${participant.participantNumber ?? "未採番"} ${participant.fullName}さんの本人連携リンクを再発行します。旧URLは使えなくなります。`)) return;
    setBusyId(participant.id);
    setMessage("");
    setIssued(null);
    const response = await fetch(`/api/admin/events/${eventId}/participants/${participant.id}/link-token`, { method: "POST" });
    ...
    setParticipants((current) => current.map((item) => item.id === participant.id
      ? { ...item, linkTokenExpiresAt: body.data.expiresAt, linkTokenUsed: false }
      : item));
  ...
  return <div className="admin-stack">
    <section className="panel wide">
      ...
      <div className="settings-grid participant-filter-controls">
        <label>参加者検索
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例: A / A01 / 氏名の一部" inputMode="search" />
        </label>
        <label>連携状態
      ...
      <p className="hint" role="status">{filtered.length}件 / 全{participants.length}件</p>

      {message && <p role="status">{message}</p>}
      {issued && <section className="configuration-complete">
        <h2>再発行済み</h2>
        <p>有効期限: {new Date(issued.expiresAt).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo", dateStyle: "short", timeStyle: "short" })}</p>
        <div className="actions">
          <a className="button-link" href={issued.url}>LINEで本人連携を開く</a>
          <button type="button" className="secondary" onClick={async () => setMessage(await copyText(issued.url) ? "本人連携リンクをコピーしました。" : "コピーできません。")}>リンクをコピー</button>
        </div>
      ...
      {filtered.length === 0
        ...
        : <div className="admin-card-list">
          {filtered.map((participant) => <article className="admin-list-card" key={participant.id}>
            <div>
              <strong>{participant.participantNumber ?? "未採番"}</strong>
              <span>{participant.fullName}</span>
            </div>
            <dl>
              <dt>LINE連携</dt><dd>{participant.linked ? "連携済み" : "未連携"}</dd>
              <dt>現在のリンク</dt><dd>{linkStatus(participant)}</dd>
            </dl>
            {participant.linked
      ...
      <div className="actions"><a className="button-link secondary" href="/admin">管理トップへ</a></div>
    </section>
  </div>;
}

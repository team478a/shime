...
export default async function EventConciergePage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const versions = await db
    ...
    .where(
      and(eq(conciergeTemplateVersions.tenantId, session.tenantId), eq(conciergeTemplateVersions.status, "published")),
    )
  ...
  const templates = templateIds.length
    ? await db
        .select({ id: conciergeTemplates.id, name: conciergeTemplates.name })
        .from(conciergeTemplates)
        .where(eq(conciergeTemplates.tenantId, session.tenantId))
    : [];

...
async function copyText(value: string) {
  try { await navigator.clipboard.writeText(value); return true; } catch {
    const textarea = document.createElement("textarea"); textarea.value = value; textarea.style.position = "fixed"; textarea.style.opacity = "0"; document.body.appendChild(textarea); textarea.select(); const copied = document.execCommand("copy"); textarea.remove(); return copied;
  }
}
...
function ApplicationFieldControl({ field }: Readonly<{ field: PublicApplicationField }>) {
  ...
  return (
    <input
      ...
      required={required}
      autoComplete={field.inputName === "fullName" ? "name" : field.inputName === "fullNameKana" ? "off" : field.inputName === "phone" ? "tel" : field.inputName === "email" ? "email" : field.inputName === "birthDate" ? "bday" : field.inputName === "nickname" ? "nickname" : field.inputName === "residenceArea" ? "address-level2" : undefined}
    />
  );
}
...
export function ApplicationForm({
  ...
  function advance() {
    ...
    if (step === 2) {
      const data = new FormData(form);
      setConfirmation(fields.map((field) => {
        const raw = String(data.get(field.inputName) ?? "");
        const value = field.type === "select" ? field.options.find((option) => option.value === raw)?.label ?? raw : raw;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const application = Object.fromEntries(
      ["fullName", "fullNameKana", "phone", "email", "birthDate", "nickname", "residenceArea", "participantCategory"]
  ...
  if (linkToken) {
    ...
    return (
      <section className="panel application-complete">
        ...
        {liffLink ? <><div className="actions"><a className="button-link" href={liffLink}>LINEで本人連携へ</a><a className="button-link secondary" href={liffLink} target="_blank" rel="noreferrer">別画面で開く</a><button type="button" className="secondary" onClick={async () => setCopyMessage(await copyText(liffLink) ? "本人連携リンクをコピーしました。" : "コピーできません。LINEで開くをお試しください。")}>リンクをコピー</button></div><p className="hint">同じスマートフォンではQRコードの読み取りは不要です。</p>{copyMessage && <ParticipantNotice>{copyMessage}</ParticipantNotice>}</> : <ParticipantNotice tone="error">LINE本人連携は現在準備中です。運営へご連絡ください。</ParticipantNotice>}
      </section>
    );
  }
    ...
    <form ref={formRef} className="application" onSubmit={submit}>
      <header className="application-header">
        ...
        <nav className="application-progress" aria-label="申込みの進行">
          <ol>{APPLICATION_STEPS.map((item, index) => <li key={item.key} className={index < step ? "complete" : ""} aria-current={index === step ? "step" : undefined}><span>{index + 1}</span>{item.label}</li>)}</ol>
        </nav>
      </header>
      ...
      <section hidden={step !== 0} data-application-step="0">
        ...
        <dl className="application-event-summary">
          <dt>開催日時</dt><dd>{eventSummary.startsAt}</dd>
          <dt>会場</dt><dd>{eventSummary.venueName}</dd>
          <dt>住所</dt><dd>{eventSummary.venueAddress}</dd>
          <dt>定員</dt><dd>{eventSummary.capacity}名</dd>
        </dl>
      ...
      <section hidden={step !== 1} data-application-step="1">
        ...
        <div className="application-fields">
          {fields.map((field) => <label key={field.fieldKey}>{field.label}{field.requirement === "required" && <span className="required-label">必須</span>}<ApplicationFieldControl field={field} /></label>)}
        </div>
      ...
      <section hidden={step !== 2} data-application-step="2">
        ...
        <label className="consent-choice application-confirmation-check">
          ...
          <span><a href={`/legal/${eventId}/event_terms?version=${encodeURIComponent(consentVersions.eventTerms)}`} target="_blank" rel="noreferrer">イベント規約</a>に同意します</span>
        </label>
        <label className="consent-choice application-confirmation-check">
          <input name="privacyAccepted" type="checkbox" required />
          <span><a href={`/legal/${eventId}/privacy?version=${encodeURIComponent(consentVersions.privacy)}`} target="_blank" rel="noreferrer">プライバシーポリシー</a>に同意します</span>
        </label>
        <p className="participant-privacy">Dreamの公開や「9999人夢応援プロジェクト」への参加は、この申込みとは別に本人が選択できます。</p>
      </section>

      <section hidden={step !== 3} data-application-step="3">
        <h2>入力内容の確認</h2>
        <p>内容を確認し、申込みを確定してください。</p>
        <dl className="application-confirmation">{confirmation.map((item) => <div key={item.label}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl>
        <p className="participant-privacy">申込み確定後、LINE本人連携へ進みます。</p>
        {error && <ParticipantNotice tone="error">{error}</ParticipantNotice>}
      </section>

      <nav className="application-actions" aria-label="申込み操作">
        {step > 0 && <button type="button" className="secondary" disabled={busy} onClick={() => { setError(""); setStep((current) => current - 1); }}>戻る</button>}
        {step < APPLICATION_STEPS.length - 1
          ? <button type="button" onClick={advance}>次へ</button>
          : <button type="submit" disabled={busy}>{busy ? "送信中…" : "申込みを確定"}</button>}
      </nav>
    </form>
  );
}

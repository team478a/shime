...
export default async function ApplyPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const participantCategories = Array.isArray(categorySource)
    ...
    : [];
  if (
    typeof eventTerms !== "string" ||
    typeof privacy !== "string" ||
    !event.venueName ||
    !event.venueAddress ||
    participantCategories.length < 2
  )
    notFound();
  ...
  if (
    !documents.some((document) => document.documentType === "event_terms" && document.version === eventTerms) ||
    !documents.some((document) => document.documentType === "privacy" && document.version === privacy)
  )
    notFound();
  const storedFields = await db
    ...
    .where(and(eq(eventFormFields.tenantId, event.tenantId), eq(eventFormFields.eventId, eventId)))
    .orderBy(asc(eventFormFields.displayOrder));
  ...
  return (
    <main>
      <ApplicationForm
        ...
        eventSummary={{
          startsAt: event.startsAt.toLocaleString("ja-JP", {
            timeZone: "Asia/Tokyo",
            dateStyle: "long",
            timeStyle: "short",
          }),

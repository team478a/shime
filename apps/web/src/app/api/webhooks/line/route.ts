...
export async function POST(request: Request) {
  ...
  const tenant = (await db.select({ id: tenants.id }).from(tenants).where(eq(tenants.code, tenantCode)).limit(1))[0];
  if (!tenant) return NextResponse.json({ code: "TENANT_NOT_FOUND" }, { status: 404 });
  let secret: string;
  try {
    secret = await getLineWebhookSecret(tenant.id);
  } catch {
    return NextResponse.json({ code: "LINE_NOT_CONFIGURED" }, { status: 503 });
  }
  if (!verifyWebhookSignature(raw, request.headers.get("x-line-signature"), secret))
  ...
  for (const event of parsed.data.events) {
    const lineUserIdHash = event.source?.userId
      ? createHash("sha256").update(`${event.source.userId}\u0000${env.SESSION_PEPPER}`).digest("hex")
      : null;
    await db

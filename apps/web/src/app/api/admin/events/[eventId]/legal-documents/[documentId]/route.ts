...
export async function PATCH(request: Request, context: Context) {
  ...
  if (document.status !== "draft")
    return NextResponse.json({ code: "DOCUMENT_IMMUTABLE", request_id: requestId }, { status: 409 });
  ...
  if (parsed.data.action === "update") {
    ...
    const [updated] = await db.transaction(async (tx) => {
      const rows = await tx
        ...
        .set({ title: data.title, body: data.body, updatedBy: session.userId, updatedAt: new Date() })
        .where(
          and(
            eq(legalDocuments.id, documentId),
            eq(legalDocuments.tenantId, session.tenantId),
    ...
    return updated
      ? NextResponse.json({ data: updated, request_id: requestId })
      : NextResponse.json({ code: "CONFLICT", request_id: requestId }, { status: 409 });
  }
  ...
  const publishResult = await db.transaction(async (tx) => {
    const rows = await tx
      .update(legalDocuments)
      .set({ status: "published", publishedAt: now, updatedBy: session.userId, updatedAt: now })
      .where(
        and(
          eq(legalDocuments.id, documentId),
          eq(legalDocuments.tenantId, session.tenantId),
    ...
    await tx
      .update(legalDocuments)
      .set({ status: "retired", updatedBy: session.userId, updatedAt: now })
      .where(
        and(
          eq(legalDocuments.tenantId, session.tenantId),
          eq(legalDocuments.eventId, eventId),
    ...
    await tx
      .update(events)
      .set({ settings: withPublishedLegalVersion(event.settings, row.documentType, row.version), updatedAt: now })
      .where(and(eq(events.id, eventId), eq(events.tenantId, session.tenantId)));
    await tx.insert(auditLogs).values({
      tenantId: session.tenantId,
      actorUserId: session.userId,

...
import { requirePermission, findMatchConflicts } from "@shime/core";
import { applications, events, getDatabase, matchCandidates, participants, preferenceSubmissions } from "@shime/db";
...
export async function GET(_request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const people = await db
    .select({ id: participants.id, participantNumber: participants.participantNumber, fullName: applications.fullName })
    .from(participants)
    .innerJoin(
      applications,

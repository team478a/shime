...
import { z } from "zod";
import { hashSessionToken, parseQrPayload, requirePermission } from "@shime/core";
...
export async function POST(request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ code: "INVALID_QR" }, { status: 400 });
  let qrToken;
  try {
    qrToken = parseQrPayload(parsed.data.qrToken);
  } catch {
    return NextResponse.json({ code: "INVALID_QR" }, { status: 400 });
  }
  ...
  return NextResponse.json({
    data: { ...target, alreadyCheckedIn: existing[0]?.status === "checked_in", checkedInAt: existing[0]?.checkedInAt },
  });
}

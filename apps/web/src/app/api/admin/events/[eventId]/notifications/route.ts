...
export async function POST(request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const recipient = await db
    .select({ id: users.id })
    .from(users)
    .where(and(eq(users.id, parsed.data.userId), eq(users.tenantId, session.tenantId)))
    .limit(1);
  if (!recipient[0]) return NextResponse.json({ code: "RECIPIENT_NOT_FOUND" }, { status: 404 });
  ...
  return created
    ...
    : NextResponse.json({ code: "DUPLICATE_NOTIFICATION" }, { status: 409 });
}

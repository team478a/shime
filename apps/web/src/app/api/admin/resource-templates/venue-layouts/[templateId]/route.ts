...
export async function PATCH(request: Request, context: Context) {
  ...
  if (parsed.data.action === "restore_as_new_version") {
    if (template.active)
      return NextResponse.json({ code: "SOURCE_VERSION_STILL_ACTIVE", request_id: requestId }, { status: 409 });
    const created = await db.transaction(async (tx) => {
      await tx.execute(
        sql`select pg_advisory_xact_lock(hashtext(${`${session.tenantId}:${VENUE_LAYOUT_MODULE_KEY}:${VENUE_LAYOUT_TEMPLATE_TYPE}:${template.templateKey}`}))`,
      );
    ...
    return NextResponse.json(
      { data: { id: created.id, name: created.name, version: created.version }, request_id: requestId },
      { status: 201 },
    );
  }

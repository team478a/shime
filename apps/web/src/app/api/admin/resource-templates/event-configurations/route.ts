...
export async function POST(request: Request) {
  ...
  const created = await db.transaction(async (tx) => {
    await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${`${session.tenantId}:${EVENT_CONFIGURATION_MODULE_KEY}:${parsed.data.templateType}:${templateKey}`}))`);
    ...
    await tx.insert(auditLogs).values({
      ...
      targetId: row.id,
      after: { moduleKey: EVENT_CONFIGURATION_MODULE_KEY, templateType: parsed.data.templateType, templateKey, version },
      requestId,
    });
    return row;
  });

  return NextResponse.json({ data: { id: created.id, name: created.name, version: created.version, templateType: created.templateType } }, { status: 201 });
}

...
export async function GET() {
  ...
  const [services, operations, schedules, templates, failedRows] =
    await Promise.all([
      db
        .select()
        .from(tenantServiceSettings)
        .where(eq(tenantServiceSettings.tenantId, session.tenantId)),
      db
        .select()
      ...
      db
        .select()
        .from(jobSchedules)
        .where(eq(jobSchedules.tenantId, session.tenantId)),
      db
        .select()
  ...
  const service = (key: string) =>
    services.find((item) => item.serviceKey === key);
  ...
  const liffId = String(
    line?.config.liffId ?? process.env.NEXT_PUBLIC_LIFF_ID ?? "",
  );
  const publicAppUrl = operations[0]?.customDomain
    ? `https://${operations[0].customDomain}`
    : process.env.APP_URL;
  ...
  return NextResponse.json({
    data: {
      line: {
        ...
        channelId: String(
          line?.config.channelId ?? process.env.LINE_CHANNEL_ID ?? "",
        ),
        liffId,
        ...lineUrls,
        secretConfigured: Boolean(
          line?.encryptedSecrets ||
      ...
      openai: {
        ...
        secretConfigured: Boolean(
          openai?.encryptedSecrets || process.env.OPENAI_API_KEY,
        ),
      ...
      schedule: schedules.find(
        (item) => item.jobKey === "notification_dispatch",
      ) ?? {
        enabled: false,
        cronExpression: "*/5 * * * *",
        timezone: "Asia/Tokyo",
...
export async function PUT(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      { code: "INVALID_INPUT", request_id: requestId },
      { status: 400 },
    );
  const db = getDatabase();
  const now = new Date();
  ...
  await db.transaction(async (tx) => {
    if (data.section === "line" || data.section === "openai") {
      ...
      const previousSecrets = current?.encryptedSecrets
        ? decryptSecrets(current.encryptedSecrets)
        : {};
      const encrypted = Object.keys(supplied).length
        ? encryptSecrets({ ...previousSecrets, ...supplied })
        : null;
      const config =
        data.section === "line"
          ? { channelId: data.channelId, liffId: data.liffId }
          : { model: data.model };
      await tx
        ...
        .values({
          ...
          config,
          encryptedSecrets:
            encrypted?.encrypted ?? current?.encryptedSecrets ?? null,
          secretFingerprint:
            encrypted?.fingerprint ?? current?.secretFingerprint ?? null,
          updatedBy: session.userId,
        })
        .onConflictDoUpdate({
          target: [
            tenantServiceSettings.tenantId,
            tenantServiceSettings.serviceKey,
          ],
          set: {
            enabled: data.enabled,
            config,

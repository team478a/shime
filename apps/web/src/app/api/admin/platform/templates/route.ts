...
export async function POST(request: Request) {
  ...
  const current = await db
    ...
    .where(
      and(
        ...
        eq(notificationTemplates.templateKey, parsed.data.templateKey),
      ),
    )
  ...
  const [created] = await db.transaction(async (tx) => {
    ...
    const rows = await tx
      ...
      .values({ tenantId: session.tenantId, ...parsed.data, version, active: true, updatedBy: session.userId })
      .returning();
    await tx.insert(auditLogs).values({
      tenantId: session.tenantId,
      actorUserId: session.userId,
      action: "notification_template.version.create",
      targetType: "notification_template",
      targetId: rows[0]?.id,
      after: { templateKey: parsed.data.templateKey, version },
      requestId,
    });
    return rows;
  });

...
async function probe(url: string, init?: RequestInit) {
  ...
  try {
    ...
    return { ok: response.ok, status: response.status, code: response.ok ? "OK" : `HTTP_${response.status}` };
  } catch (error) {
    return {
      ok: false,
      status: 0,
      code: error instanceof Error && error.name === "AbortError" ? "TIMEOUT" : "CONNECTION_FAILED",
    };
  } finally {
    clearTimeout(timer);
  }
...
export async function POST(request: Request) {
  ...
  let result: { ok: boolean; status: number; code: string };
  if (target === "line" || target === "openai") {
    const setting = (
      await db
        .select()
        .from(tenantServiceSettings)
        .where(and(eq(tenantServiceSettings.tenantId, session.tenantId), eq(tenantServiceSettings.serviceKey, target)))
        .limit(1)
    )[0];
    ...
    try {
      ...
      return NextResponse.json({ code: "SECRET_DECRYPTION_FAILED" }, { status: 500 });
    }
    if (target === "line") {
      const token = secrets.accessToken ?? process.env.LINE_CHANNEL_ACCESS_TOKEN;
      result = token
        ? await probe("https://api.line.me/v2/bot/info", { headers: { Authorization: `Bearer ${token}` } })
        : { ok: false, status: 0, code: "CREDENTIAL_MISSING" };
    } else {
      const apiKey = secrets.apiKey ?? process.env.OPENAI_API_KEY;
      result = apiKey
        ? await probe("https://api.openai.com/v1/models", { headers: { Authorization: `Bearer ${apiKey}` } })
        : { ok: false, status: 0, code: "CREDENTIAL_MISSING" };
    }
    await db
    ...
    const url =
      target === "domain"
        ? operations?.customDomain
          ? `https://${operations.customDomain}/api/health`
          : null
        : (operations?.healthcheckUrl ?? `${process.env.APP_URL}/api/health`);

...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { code: "INVALID_INPUT", message: "入力内容を確認してください", request_id: requestId },
      { status: 400 },
    );
  }
  const env = getEnv();
  ...
  const valid =
    match &&
    !isLocked(match.lockedUntil) &&
    (await verifyPassword(match.passwordHash, parsed.data.password, env.PASSWORD_PEPPER));
    ...
    await db.transaction(async (tx) => {
      ...
      if (match) {
        ...
        await tx
          ...
          .set({ failedAttempts: failures, lockedUntil: nextLockout(failures), updatedAt: new Date() })
          .where(eq(passwordCredentials.userId, match.userId));
      }
    });
    return NextResponse.json(
      { code: "INVALID_CREDENTIALS", message: "ログインIDまたはパスワードが違います", request_id: requestId },
      { status: 401 },
    );
  }
...
export async function DELETE() {
  ...
  if (rawToken) {
    await getDatabase()
      ...
      .where(eq(staffSessions.tokenHash, (await import("@shime/core")).hashSessionToken(rawToken, env.SESSION_PEPPER)));
  }

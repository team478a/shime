...
export async function GET() {
  ...
  const templates = await db.select().from(conciergeTemplates).where(and(
  ...
  const versions = await db.select().from(conciergeTemplateVersions).where(
    eq(conciergeTemplateVersions.tenantId, session.tenantId),
  ).orderBy(desc(conciergeTemplateVersions.version));
  return NextResponse.json({ data: templates.map((template) => ({
    ...template,
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ code: "INVALID_INPUT", issues: parsed.error.issues.map((issue) => issue.message), request_id: requestId }, { status: 400 });
  const db = getDatabase();
  const created = await db.transaction(async (tx) => {
    const templateKey = templateKeyForName(parsed.data.name);
    await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${`${session.tenantId}:${CONCIERGE_MODULE_KEY}:${parsed.data.templateId ?? templateKey}`}))`);
    let template = parsed.data.templateId
      ? (await tx.select().from(conciergeTemplates).where(and(
    ...
    if (!template) {
      [template] = await tx.insert(conciergeTemplates).values({
    ...
    const latest = await tx.select({ version: conciergeTemplateVersions.version }).from(conciergeTemplateVersions).where(and(
      eq(conciergeTemplateVersions.tenantId, session.tenantId),
      eq(conciergeTemplateVersions.templateId, template.id),
    )).orderBy(desc(conciergeTemplateVersions.version)).limit(1);
    const [version] = await tx.insert(conciergeTemplateVersions).values({
      tenantId: session.tenantId,

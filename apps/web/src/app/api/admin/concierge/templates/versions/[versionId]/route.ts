...
export async function PATCH(request: Request, { params }: { params: Promise<{ versionId: string }> }) {
  ...
  if (!parsed.success) return NextResponse.json({ code: "INVALID_INPUT", issues: parsed.error.issues.map((issue) => issue.message) }, { status: 400 });
  const { versionId } = await params;
  const db = getDatabase();
  const [updated] = await db.transaction(async (tx) => {
    const rows = await tx.update(conciergeTemplateVersions).set({
      payload: parsed.data,
      updatedAt: new Date(),
    }).where(and(
  ...
  return updated ? NextResponse.json({ data: updated }) : NextResponse.json({ code: "DRAFT_NOT_FOUND" }, { status: 409 });
}

...
export async function POST(_request: Request, { params }: { params: Promise<{ versionId: string }> }) {
  ...
  const rows = await db.select().from(conciergeTemplateVersions).where(and(
  ...
  const cards = cardIds.length ? await db.select({ id: conciergeCardAssetVersions.id }).from(conciergeCardAssetVersions).where(and(
  ...
  if (cards.length !== cardIds.length) issues.push({ code: "CARD_MAPPING_INVALID", message: "公開済みでないカード画像が含まれています。" });
  if (issues.length) return NextResponse.json({ code: "PUBLISH_VALIDATION_FAILED", issues }, { status: 409 });
  const now = new Date();
  await db.transaction(async (tx) => {
    await tx.update(conciergeTemplateVersions).set({ status: "archived", archivedAt: now, updatedAt: now }).where(and(
      eq(conciergeTemplateVersions.tenantId, session.tenantId),
      eq(conciergeTemplateVersions.templateId, version.templateId),
      eq(conciergeTemplateVersions.status, "published"),
    ));
    await tx.update(conciergeTemplateVersions).set({ status: "published", publishedAt: now, updatedAt: now }).where(and(
      eq(conciergeTemplateVersions.id, versionId),
      eq(conciergeTemplateVersions.tenantId, session.tenantId),
      eq(conciergeTemplateVersions.status, "draft"),
    ));

...
export async function GET(_request: Request, { params }: { params: Promise<{ versionId: string }> }) {
  ...
  const row = (
    await getDatabase()
      .select({ objectKey: conciergeCardAssetVersions.storageObjectKey })
      .from(conciergeCardAssetVersions)
      .where(
        and(eq(conciergeCardAssetVersions.id, versionId), eq(conciergeCardAssetVersions.tenantId, session.tenantId)),
      )
      .limit(1)
  )[0];

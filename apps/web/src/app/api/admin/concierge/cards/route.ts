...
export async function GET() {
  ...
  const assets = await db
    ...
    .where(
      and(eq(conciergeCardAssets.tenantId, session.tenantId), eq(conciergeCardAssets.moduleKey, CONCIERGE_MODULE_KEY)),
    )
...
export async function POST(request: Request) {
  ...
  if (!parsed.success || !(file instanceof File))
    return NextResponse.json({ code: "INVALID_INPUT", request_id: requestId }, { status: 400 });
  let image;
  try {
    ...
    if (error instanceof ConciergeImageValidationError)
      return NextResponse.json({ code: error.code, request_id: requestId }, { status: 400 });
    throw error;
  }
  const db = getDatabase();
  ...
  if (duplicate[0])
    return NextResponse.json(
      { code: "IMAGE_DUPLICATE", duplicateVersionId: duplicate[0].id, request_id: requestId },
      { status: 409 },
    );
  let asset = parsed.data.assetId
    ? (
        await db
          .select()
          .from(conciergeCardAssets)
  ...
  if (parsed.data.assetId && !asset)
    return NextResponse.json({ code: "ASSET_NOT_FOUND", request_id: requestId }, { status: 404 });
  const assetId = asset?.id ?? randomUUID();
  const latest = asset
    ? await db
        .select({ version: conciergeCardAssetVersions.version })
        .from(conciergeCardAssetVersions)
        .where(
          and(
            eq(conciergeCardAssetVersions.tenantId, session.tenantId),
  ...
  await createConciergeStorageProvider().uploadImmutable({
    objectKey,
    bytes: image.bytes,
  ...
  const created = await db.transaction(async (tx) => {
    ...
    const [version] = await tx
      .insert(conciergeCardAssetVersions)
      .values({
        tenantId: session.tenantId,
        assetId: asset.id,

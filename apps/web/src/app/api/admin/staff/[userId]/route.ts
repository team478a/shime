...
export async function PATCH(request: Request, { params }: { params: Promise<{ userId: string }> }) {
  ...
  const rows = await db
    ...
    .limit(1);
  const target = rows[0];
  if (!target) return NextResponse.json({ code: "NOT_FOUND", request_id: requestId }, { status: 404 });
  ...
  await db.transaction(async (tx) => {
    await tx
      .update(users)
      .set({ displayName: parsed.data.displayName, status: parsed.data.status, updatedAt: new Date() })
      .where(and(eq(users.id, userId), eq(users.tenantId, session.tenantId)));
    await tx
      .update(staffRoles)
      .set({ role: parsed.data.role, updatedAt: new Date() })
      .where(and(eq(staffRoles.userId, userId), eq(staffRoles.tenantId, session.tenantId), isNull(staffRoles.eventId)));
    if (passwordHash)
      await tx
    ...
    if (passwordHash || parsed.data.status !== "active" || parsed.data.role !== target.role)
      await tx

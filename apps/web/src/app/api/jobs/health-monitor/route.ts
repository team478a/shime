...
async function run(request: Request) {
  ...
  if (!expected || !hasValidBearerSecret(request.headers.get("authorization"), expected))
  ...
  for (const item of settings) {
    const url = item.healthcheckUrl ?? `${process.env.APP_URL}/api/health`;
    ...
    await db
      ...
      .set({
        ...
        lastRunSummary: { healthy: ok ? 1 : 0, failed: ok ? 0 : 1 },
        updatedAt: new Date(),
      })
      .where(and(eq(jobSchedules.tenantId, item.tenantId), eq(jobSchedules.jobKey, "health_monitor")));
  }

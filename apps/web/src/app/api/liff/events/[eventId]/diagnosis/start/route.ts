...
import { startDiagnosis } from "@shime/web/server/concierge-diagnosis-use-cases";

const bodySchema = z.object({ restart: z.boolean().optional() }).default({});
...
export const POST = participantHandler(
  ...
  async ({ eventId, participant, requestId, session }, request) => {
    ...
    const result = await startDiagnosis.execute(
      {

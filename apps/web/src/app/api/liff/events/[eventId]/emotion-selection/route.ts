...
import { z } from "zod";
import { emotionCards, emotionSelections, getDatabase, participants } from "@shime/db";
...
export async function POST(request: Request, { params }: { params: Promise<{ eventId: string }> }) {
  ...
  const [saved] = existing[0]
    ? await db.update(emotionSelections).set(values).where(eq(emotionSelections.id, existing[0].id)).returning()
    : await db
        .insert(emotionSelections)
        .values({ tenantId: auth.session.tenantId, eventId, participantId: auth.participant.id, ...values })
        .returning();
  await db
    .update(participants)

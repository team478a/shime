...
export async function PUT(request: Request) {
  ...
  const auth = await requireParticipantForEvent(parsed.data.eventId).catch(() => null);
  if (!auth) return NextResponse.json({ code: "UNAUTHORIZED" }, { status: 401 });
  let dreamText;
  try {
    dreamText = validateDreamText(parsed.data.dreamText);
  } catch {
  ...
  const existing = await db
    .select()
    .from(dreamProfiles)
    .where(and(eq(dreamProfiles.tenantId, auth.session.tenantId), eq(dreamProfiles.userId, auth.session.userId)))
    .limit(1);
  const settings = await db
    .select()
    .from(eventDreamSettings)
    .where(
      and(eq(eventDreamSettings.tenantId, auth.session.tenantId), eq(eventDreamSettings.eventId, parsed.data.eventId)),
    )
    .limit(1);
  if (parsed.data.projectOptIn && !settings[0]?.projectConsentVersion)
    return NextResponse.json({ code: "PROJECT_CONSENT_NOT_CONFIGURED" }, { status: 409 });
  const [profile] = await db.transaction(async (tx) => {
    const rows = existing[0]
      ? await tx
          .update(dreamProfiles)
          .set({ dreamText, visibility: parsed.data.visibility, confirmedAt: new Date(), updatedAt: new Date() })
          .where(eq(dreamProfiles.id, existing[0].id))
          .returning()
      : await tx
    ...
    await tx
      ...
      .where(eq(participants.id, auth.participant.id));
    if (parsed.data.projectOptIn && settings[0]?.projectConsentVersion)

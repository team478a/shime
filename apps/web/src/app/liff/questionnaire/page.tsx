...
export default function QuestionnairePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/liff/events/${eventId}/questionnaire`)
      ...
      .then(({ response, body }) => {
        ...
        setAnswers(Object.fromEntries(body.data.answers.map((answer: QuestionnaireAnswer) => [answer.questionId, answer])));
      })
  ...
  function toggle(question: Question, code: string) {
    setAnswers((current) => {
      ...
      const optionCodes = old.optionCodes.includes(code)
        ...
        : question.maxSelections === 1 ? [code] : [...old.optionCodes, code].slice(0, question.maxSelections);
  ...
  async function submit() {
    if (!isQuestionnaireComplete(questions.map((question) => question.id), answers)) {
  ...
  return <main><section className="panel wide questionnaire participant-content">
    <ParticipantPageHeader eyebrow="SEATING GUIDE" title="席案内のための5問" description="心地よく会話できる席をご案内するための質問です。" current="questionnaire" eventId={eventId} />
    ...
    {(!eventId || loadState === "error") && <ParticipantNotice tone="error">質問を読み込めませんでした。LINEの案内から開き直してください。</ParticipantNotice>}
    {loadState === "loaded" && questions.length === 0 && <ParticipantNotice>質問はまだ準備されていません。</ParticipantNotice>}
    {loadState === "loaded" && questions.map((question, index) => {
      const answer = answers[question.id];
      return <fieldset key={question.id} disabled={submitted || busy}>
        <legend>{index + 1}. {question.prompt}</legend>
        <div className="option-grid">{question.options.map((option) => <label key={option.code} className="choice">
          <input type={question.maxSelections === 1 ? "radio" : "checkbox"} name={question.id} checked={answer?.optionCodes.includes(option.code) ?? false} onChange={() => toggle(question, option.code)} />{option.label}
        </label>)}</div>
        <label className="choice decline"><input type="checkbox" checked={answer?.declined ?? false} onChange={() => decline(question.id)} />答えたくない</label>
        <small>最大{question.maxSelections}個</small>
      </fieldset>;
    })}
    {loadState === "loaded" && questions.length > 0 && !submitted && <div className="actions">
      <button type="button" className="secondary" onClick={save} disabled={busy}>{busy ? "保存中…" : "途中保存"}</button>
      <button type="button" onClick={submit} disabled={busy}>{busy ? "提出中…" : "5問を提出"}</button>
    </div>}
    {submitted && <a className="button-link" href={`/liff/passport?eventId=${eventId}`}>SHIME® PASSへ</a>}

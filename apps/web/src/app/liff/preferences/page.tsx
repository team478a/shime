  ...
  useEffect(() => {
    ...
    fetch(`/api/liff/events/${eventId}/preference-options`)
      ...
      .then(({ response, body }) => {
        ...
        setNotes(
          Object.fromEntries(choices.map((choice: Choice) => [choice.toParticipantId, choice.privateNote ?? ""])),
        );
    ...
    <main>
      <section className="panel wide questionnaire participant-content">
        <ParticipantPageHeader
          eyebrow="PREFERENCE"
          title="またお話ししたい方"
          description="イベントで感じた気持ちを、ご自身のペースで選択できます。"
          current="preference"
          eventId={eventId}
        />
        ...
        {(!eventId || loadState === "error") && (
          <ParticipantNotice tone="error">希望入力期間外、または候補を読み込めませんでした。</ParticipantNotice>
        )}
        {loadState === "loaded" && (
          <>
              ...
              {candidates.map((candidate) => {
                ...
                return (
                  <div className={index >= 0 ? "emotion-card selected" : "emotion-card"} key={candidate.id}>
                    ...
                    {index >= 0 && (
                      <textarea
                        aria-label="運営だけが確認するメモ"
                        placeholder="任意メモ（相手には非公開）"
                        value={notes[candidate.id] ?? ""}
                        onChange={(event) => setNotes((old) => ({ ...old, [candidate.id]: event.target.value }))}
                        maxLength={1000}
                      />
                    )}
                  </div>
            ...
            {candidates.length === 0 && (
              <p className="participant-empty">現在選択できる方はいません。「今回は選ばない」として提出できます。</p>
            )}
            <div className="actions">
              ...
              <button type="button" onClick={submit} disabled={busy}>
                {selected.length ? "この内容で提出" : "今回は選ばず提出"}
              </button>
            </div>
          </>
        )}
